...
def draw_chum(x, y, R, width):
    ...
    dr.rect(screen, almost_black, (x - 5 * R / 3, y, 10 * R / 3, 4 * R / 3))


draw_chum(200, 550, 160, 3)
...
def draw_cat(x, y, k):
    ...
    draw_fish(x - 45 * k, y - 10 * k, 1.2)  # fish
    ...
    dr.circle(screen, black, (x + 53 * k / 4, y - 23 * k), 5)

    dr.ellipse(screen, white, (x + 100 * k / 4, y - 25 * k, 21 * k, 14 * k))  # eye 2
    dr.circle(screen, black, (x + 165 * k / 4, y - 18 * k), 5)

    dr.circle(screen, black, (x + 40 * k / 4, y - 6 * k), 3)  # nose


def draw_chuckcha(x, y, k):
    dr.ellipse(screen, areal, (x - 10 * k, y - 90 * k, 220 * k, 150 * k))  # areal 1
    dr.ellipse(screen, (138, 124, 101), (x + 10 * k, y - 70 * k, 180 * k, 120 * k))  # areal 1

    dr.ellipse(screen, brown, (x, y, 200 * k, 350 * k))  # body
    dr.rect(screen, white, (x, y + 175 * k, 200 * k, 175 * k))  # hide
    ...
    dr.ellipse(screen, light_brown, (x + 23 * k, y - 53 * k, 150 * k, 90 * k))  # head
    dr.arc(screen, black, (x + 23 * k, y + 1 * k, 150 * k, 90 * k), 3 * pi / 10, 7 * pi / 10, width=k * 3)  # mouth

    dr.line(screen, black, (x + 50 * k, y - 30 * k), (x + 80 * k, y - 20 * k), width=3 * k)  # eye 1
    dr.line(screen, black, (x + 150 * k, y - 30 * k), (x + 120 * k, y - 20 * k), width=3 * k)  # eye 2

    dr.line(screen, black, (x - 55 * k, y + 230 * k), (x - 55 * k, y - 60 * k), width=2 * k)  # stick


draw_chum(200, 550, 160, 3)
draw_cat(130, 750, 1)
draw_chuckcha(500, 500, 1)
